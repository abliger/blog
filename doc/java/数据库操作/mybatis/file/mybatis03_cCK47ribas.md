...
@Select("select stu.id,stu.name,tea.id tid,tea.name tname\n" +
        ...
        "where stu.id = #{id}")
Student queryStuAndTeaById(@Param("id") Integer id);
```
...
@Results({
    ...
    @Result(column = "teacher_id",
            property = "teacher",
            one = @One(select = "com.atguigu.mapper.TeacherMapper.queryTeacherById",fetchType = FetchType.DEFAULT))
})
@Select("select id,name,teacher_id from t_student where id = #{id}")
Student queryStuAndTeaByIdTwo(@Param("id") Integer id);
```
...
List<Passenger> queryPassengersByBId(Integer bid);
```
...
@Results({
    ...
    @Result(column = "id",
            property = "passengers",
            many = @Many(select = "com.atguigu.mapper.PassengerMapper.queryPassengersByBId",fetchType = FetchType.LAZY))
})
...
@Results({
    ...
    @Result(column = "{bid=id,name=name}",
            property = "passengers",
            many = @Many(select = "com.atguigu.mapper.PassengerMapper.queryPassengerByBidOrLikeName",fetchType = FetchType.LAZY))
})
@Select(" select `id`,name from t_bus where id = #{id}")
...
public interface UserMapper {
     ...
     * @return
     */
    List<User> selectUserExampleByNameOrSex(User user);
}
```
...
public void selectUserById(){
    ...
    List<User> list = userMapper.selectUserExampleByNameOrSex(new User("b", 0));
    list.forEach(user -> {
...
<!-- ![](mybatis03.assets\无标题.png) -->

 ```java
@Test
public void selectUser() {
    List<User> list = userMapper.selectUser(1);
    List<User> list1 = userMapper.selectUser(1);
    System.err.println(list);
...
public void queryOne(){
    ...
    try {
        ...
        List<User> list = userMapper.selectUserById(1);
        System.err.println(list);
    } finally {
        session.close();
    }
...
public void firstLevelCacheFail2() {
    ...
    List<User> list = mapper.selectUser(1);
    List<User> list1 = mapper.selectUser(1);
    System.err.println(list1);
    List<User> list2 = mapper.selectUser(2);
...
public void firstLevelCacheFail4() {
    ...
    List<User> list1 = mapper.selectAll();
    System.err.println(list1);
}
```
