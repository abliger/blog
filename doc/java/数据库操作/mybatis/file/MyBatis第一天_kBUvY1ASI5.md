...
<!-- ![](MyBatis第一天.assets\无标题-1597979851242.png) -->
...
## 3 在源码目录下创建 mybatis 的核心配置文件 mybatis-config.xml
...
<!-- ![image-20200729112344653](MyBatis第一天.assets\image-20200729112344653.png) -->
...
# 5、mybatis 的核心配置之 settings

这是 MyBatis 中极为重要的调整设置，它们会改变 MyBatis 的运行时行为。下表描述了设置中各项的意图、默认值等。

## 5.1、所有 mybatis 的 settings 设置选项
...
<!-- ![image-20200821103650884](MyBatis第一天.assets\image-20200821103650884.png) -->

# 6、mybatis 的核心配置之 typeAliases
...
## 6.2、mybatis 的核心配置之 typeHandlers
...
# 7、mybatis 的核心配置之 environments

目的:解决数据库环境

<!-- ![](MyBatis第一天.assets\无标题-1597991428603.png) -->

environments 标签配置的是数据库环境
...
# 8、mybatis 的核心配置之 databaseIdProvider
...
1:由于不同的数据库sql语句略有不同,我们称之为方言
2:mybatis给我们提供databaseIdProvider标签可以连接不同的数据库
3:在sql标签中可以使用datasourceId指定
4:当sql标签中有重名的id时报错
5:加入databaseId="mysql"时,id允许重复,mybatis会加载带有匹配databaseId和不带有databaseId的标签
6:优先使用databaseId
7:当有一个databaseId="oracle"不匹配时,会优先加载匹配上的
...
<!-- ![](MyBatis第一天.assets\无标题-1597997134655.png) -->
...
public class PJL implements WP{

    @Override
    public void throwEye() {
        System.err.println("pjl抛媚眼");
    }

    @Override
    public void gotoRoom() {
        System.err.println("pjl去房间");
    }
}
...
public class XMQ {
    ...
    public static void main(String[] args) {
       /* WP wp = new PJL();
        System.err.println("洗白白");
        wp.gotoRoom();
        System.err.println("还得洗一遍");*/
        WP wp = new PJL();
        //代理
        /**
         * 1:类加载器
         ...
         * 3:代理策略
         */
       WP wpProxy = (WP) Proxy.newProxyInstance(wp.getClass().getClassLoader(),
               wp.getClass().getInterfaces(),
               new InvocationHandler() {
            /**
             *
             * @param obj     不能用,代理对象实例
             * @param method    代理方法反射对象
             * @param args      方法参数
             * @return
             * @throws Throwable
             */
            ...
            public Object invoke(Object obj,
                ...
                System.err.println("洗白白");
                //执行代理对象方法,返回代理对象
                Object proxy = method.invoke(wp);
                System.err.println("里里外外都得洗");
                return proxy;
            }
        });
        //代理对象执行gotoRoom方法
...
<!-- ![](MyBatis第一天.assets\无标题-1597806911378.png) -->
