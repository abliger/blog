...
例如: 设 $\Sigma = \lbrace a ,b  \rbrace$,则  $$\Sigma^* = \lbrace \varepsilon,a,b,aa,ab,ba,bb,aab,abb,...\rbrace \\$$
...
文法分为四种、即 0型（无限制）文法、1型（上下文有关）文法、2型（上下文无关）文法和3型（正则）文法.
...
如果对 0 型文法施加以下 i 条限制,就得到了 i 型文法:
