...
export function lex(code: string) {
    ...
    while (current < code.length) {
        ...
        if (('a' <= char && char <= 'z') || ('A' <= char && char <= 'Z')) {
            var temp = []
            while (current < code.length && (('a' <= code[current] && code[current] <= 'z') || ('A' <= code[current] && code[current] <= 'Z') || ('0' <= code[current] && code[current] <= '9') || code[current] === "_")) {
                temp.push(code[current])
                current++
            }
            const identifier = temp.join("")
            if (/^(let|var|const|class|static|pub|fun|return|break|continue|default|else|for|if|in|of|switch|while|struct)$/.test(identifier)) {
        ...
        if (('0' <= char && char <= '9') || char === ".") {
            ...
            temp.push(code[current])
            current++
            if (code[current] === "b" || code[current] === "o" || code[current] === "x") {
                temp.push(code[current])
                current++
                while (current < code.length && ('0' <= code[current] && code[current] <= '9') || code[current] === "_" || code[current] === ".") {
