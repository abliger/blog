<template>
    <div ref="el" class="ShuffleContent">
        <slot></slot>
    </div>
</template>
<script lang="tsx" setup>
    ...
    const props = defineProps<{
        effect: 'fx1' | 'fx2' | 'fx3' | 'fx4' | 'fx5' | 'fx6'
    }>()
    ...
    class Cell {
        ...
        constructor(
            DOM_el,
            { position, previousCellPosition } = {} as {
                position: number
        ...
        fx5() {
            ...
            const loop = (line, cell, iteration = 0) => {
                ...
                if (iteration === MAX_CELL_ITERATIONS - 1) {
                    ...
                    cell.color =
                        line.cells[cell.previousCellPosition].cache.color
                    cell.DOM.el.style.color = cell.color
                }
    ...
    onMounted(() => {
        const textElement = document.querySelector('.ShuffleContent')
        const ts = new TypeShuffle(textElement)
        const observer = new IntersectionObserver(
            entries => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ts.trigger(props.effect)
                        // 只触发一次动画
